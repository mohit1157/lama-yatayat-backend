...
func main() {
	...
	zapLog := logger.New("connection-service", cfg.App.Debug)
	defer zapLog.Sync()
	...
	r.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "connection-service"})
	})

	// TODO: Register service-specific routes here
	// See internal/connection-service/ for handler, service, repository layers
	api := r.Group("/api/v1")
	_ = api
	...
	go func() {
		log.Printf("🚀 connection-service starting on :%s", port)
