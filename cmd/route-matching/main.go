...
func main() {
	...
	zapLog := logger.New("route-matching", cfg.App.Debug)
	defer zapLog.Sync()
	...
	r.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "route-matching"})
	})

	// TODO: Register service-specific routes here
	// See internal/route-matching/ for handler, service, repository layers
	api := r.Group("/api/v1")
	_ = api
	...
	go func() {
		log.Printf("🚀 route-matching starting on :%s", port)
