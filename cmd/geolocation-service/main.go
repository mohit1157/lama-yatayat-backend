...
func main() {
	...
	zapLog := logger.New("geolocation-service", cfg.App.Debug)
	defer zapLog.Sync()
	...
	r.GET("/health", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"status": "ok", "service": "geolocation-service"})
	})

	// TODO: Register service-specific routes here
	// See internal/geolocation-service/ for handler, service, repository layers
	api := r.Group("/api/v1")
	_ = api
	...
	go func() {
		log.Printf("🚀 geolocation-service starting on :%s", port)
